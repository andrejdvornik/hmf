...
class MassFunction(transfer.Transfer):
    ...
    def nu_fn(self):
        ...
        # Identical methods, bottom one allows nd arrays
        return make_interp_spline(np.squeeze(self.m), self.nu, k=5, axis=-1)
    ...
    def mass_nonlinear(self):
        ...
        for i in range(self.z.size):
            if nu[i, :].min() > 1 or nu[i, :].max() < 1:
                ...
                if res.success:
                    r = np.exp(res.x[0])
                    nonlinear[i] = self.filter.radius_to_mass(r, self.mean_density0)
                else:
    ...
    def _gtm(self, dndm_in, mass_density=False):
        """
